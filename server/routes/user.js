    ...
    async function(err, locations) {
      if (err) console.error(err);
        ...
        if (locations.length == 0) {
          return res.json({
            message:
              "sorry your request could not be processed there are no nearby drivers"
          });
        }
        ...
        for (let i = 0; i < drivers.length; i++) {
          ...
          try {
            ...
            console.log(
              "emitting to driver with socket id" + drivers[i].socketId
            );
            io.to(`${drivers[i].socketId}`).emit("new-booking", {
              bookingData: book
            });
          } catch (error) {
...
router.post("/book/deliver", async (req, res) => {
  ...
  return console.log(data);
  console.log("got the booking data" + data);
  //create the book model
  const book = new bookModel({
    bookInfo: {
      bookedUser: {
