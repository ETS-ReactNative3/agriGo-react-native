...
router.post("/profile", async (req, res) => {
  ...
  foundDriver.machinery = req.body.machinery.split(",");

  try {
    let updateDriver = await foundDriver.save();
    return res.json({ message: "updated driver profile" });
  } catch (e) {
...
const sendRealtimeUpdate = (userid, io, driverSocketId) => {
  ...
  locationUpdateInterval = setInterval(async () => {
    ...
    console.log(
      "sending driver location " + i + " to user socket " + foundUser.socketId
    );
